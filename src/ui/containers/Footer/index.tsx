                ...
                {leftLinks.map(({ label, href }, idx) => (
                  <li key={idx} className="mb-2 last:mb-0">
                    <Link
                      ...
                      className="text-18px600 font-inter hover:underline"
                    >
                      {label}
                    </Link>
                  </li>
            ...
            <div className="text-center mb-6 md:mb-0">
              <p className="mb-2">
                <Link
                  ...
                  className="text-20px500 font-inter opacity-[64%] hover:underline"
                >
                  info@aztelco.az
                </Link>
              </p>
              <p className="text-48px700 font-bai my-4">+994 12 345 67 89</p>
              <p className="text-20px500 font-inter opacity-[64%]">
                {t("address")}
              </p>
                ...
                {rightLinks.map(({ label, href }, idx) => (
                  <li key={idx} className="mb-2 last:mb-0">
                    <Link
                      locale={locale}
                      href={href as typeof Link.prototype.href}
                      className="font-inter text-18px600 hover:underline"
                    >
                      {label}
                    </Link>
                  </li>
              ...
              {[...leftLinks, ...rightLinks].map(({ label, href }, idx) => (
                <li key={idx}>
                  <Link
                    locale={locale}
                    href={href as typeof Link.prototype.href}
                    className="text-base font-inter hover:underline"
                  >
                    {label}
                  </Link>
                </li>
